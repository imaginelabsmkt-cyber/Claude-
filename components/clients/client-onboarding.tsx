...
export function ClientOnboarding({
  ...
  // Começa em modo LEITURA quando já há DNA preenchido (fica bonito, não
  // parece formulário). Sem dados, abre direto na edição.
  const temDados = Object.values(inicial).some((v) => v?.trim());
  ...
  // ---------- MODO LEITURA (visualização bonita do DNA) ----------
  if (modo === "ver") {
    const secoesComDados = ONBOARDING_SECOES.map((secao) => ({
      secao,
      campos: secao.campos.filter((c) => valores[c.id]?.trim()),
    })).filter((s) => s.campos.length > 0);

    const marca = valores.marca?.trim();
    const segmento = valores.segmento?.trim();

    return (
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-3 rounded-2xl bg-gradient-to-br from-brand-700 to-brand-800 p-5 text-white">
          <div>
            <p className="text-xs font-semibold uppercase tracking-widest text-white/60">
              DNA do cliente
            </p>
            <h2 className="mt-1 text-2xl font-bold">{marca || "Cliente"}</h2>
            {segmento ? (
              <p className="mt-0.5 text-sm text-white/80">{segmento}</p>
            ) : null}
          </div>
          <button
            type="button"
        ...
        {secoesComDados.length === 0 ? (
          <p className="rounded-2xl border border-dashed border-gray-300 bg-white p-6 text-center text-sm text-gray-400">
            Ainda não há informações preenchidas. Clique em “Editar” para
            começar.
          </p>
        ) : (
          secoesComDados.map(({ secao, campos }) => (
            <div
              key={secao.id}
